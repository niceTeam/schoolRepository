...
public interface GroupChatMapper {
	...
	@Select("SELECT * FROM `groupchat` WHERE gradeId = #{gradeId} AND groupId IN("
			+ "	SELECT groupId FROM `groupmember` WHERE userType=1 AND userId = #{stuId})")
	List<GroupChat> listGroupChatbyStuIdAndGradeId(@Param("stuId") Integer stuId, @Param("gradeId") Integer gradeId);
}